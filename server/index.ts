...
app.post("/register", (req, res) => {
  bcrypt
    ...
    .then(async (hashedPassword) => {
      ...
      else res.status(401).send({ message: "User already exist!" });
    });
});
...
app.get("/auth/google/callback", async (req, res) => {
  ...
  if (!user) {
    await collection
      .insertOne({ email: googleUser.email, name: googleUser.name })
      .then((r) => {
...
app.get("/profile", authorization, async (req, res) => {
  ...
  if (token) {
    const decoded: { _id?: ObjectId } = jwt.verify(
      token,
      JWT_SECRET as string,
    ) as { _id?: ObjectId };
    if (decoded) {
      const user = await collection.findOne({ _id: new ObjectId(decoded._id) });
      res.send({ email: user?.email, name: user?.name }).status(200);
    } else return res.status(401).json({ message: "can't find user" });
  } else
